...
description: 數題數題 (特教數學學習單生成器) - 特教數學題目轉點讀報讀 HTML 網頁與原生 OMML Word 檔生成器。當使用者提供數學題目（或圖片說明）並說「數題數題」、「生成同類型題目」、「報讀HTML」、「特教學習單」或「生成Word檔」時觸發。Agent 會先以 ask_question 主動詢問題目數量與挖空鷹架策略（漸進式褪除或固定一致），產出包含 💡觀念提醒框、特教友善、A4直印優化且內建 Web Speech API 語音點讀報讀的 HTML 學習單網頁，並同時自動匯出含微軟原生 OMML 方程式物件的高品質 Word (.docx) 檔。
---
...
2. 根據使用者回答，產出同題型、同步驟的練習題，並同時輸出：
   - **語音點讀報讀 HTML 網頁**（頂部含 **💡觀念提醒框**、內建 Web Speech API 點讀工具列，點擊題目或算式即可語音發音，預設 `@media print` 隱藏報讀控制列，高版面密度可直印 A4）。
   - **Microsoft Word 檔 (.docx)**（頂部含 **💡觀念提醒框**、數學式全部為微軟 Word 方程式編輯器原生 OMML 物件，非圖片、非斜線純文字）。
...
# 💡 二、頂部「觀念提醒框」規範 (Concept Box)

在學習單/考卷**頂部（學生資料欄下方、第 1 題之前）**，必須包含一個特教友善的 **💡 觀念提醒框**：

1. **文字內容**：
   - 標題：`💡 觀念提醒：`
   - 使用極簡、生活化、口語化的幾句話比喻解題核心觀念（例如：`移項時「減號」搬過等號要變成「加號」！數字黏在 x 前面時，搬過去對面要放在分母喔！`）。
2. **HTML 呈現**：
   - 使用 `<div class="concept-box">` 樣式（圓角框、淡藍/灰色背景 `bg-slate-50`、深色邊框 `border-2 border-slate-400`）。
   - 內建 `class="read-sentence"` 與 `data-read-text` 支援點擊語音朗讀。
3. **Word (.docx) 呈現**：
   - 使用頂部高亮淺色儲存格背景、深色邊框表格呈現 `💡 觀念提醒：` 區塊。
...
# 🔊 四、HTML 語音點讀報讀規範 (Point-to-Read TTS)

1. **觀念框、句子與算式點讀包覆**：
   - 觀念提醒框、所有句子、題目與算式步驟皆使用 `<span class="read-sentence" data-read-text="..." onclick="speakSentence(this, event)">` 包覆。
...
2. **HTML 列印設定**：
   ...
   - `.problem-card` 與 `.concept-box`：`break-inside: avoid; page-break-inside: avoid;`
...
當使用者請求時，請一次產出並提供：
1. **完整 點讀報讀 HTML 檔**（含頂部 💡觀念提醒框、Web Speech API 語音發音與可直接 Ctrl+P 列印）
2. **Python 腳本 (於 scratch 目錄下執行)**，產出 **完整 Word (.docx) 檔**（含頂部 💡觀念提醒框、原生 OMML 數學方程式、標楷體、特教卡片框）
